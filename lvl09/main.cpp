...
void IterateOverMap(vector<vector<Point>> &map, function<void(vector<vector<Point>> &map, int row, int col)> op)
{
...
int main(){
    ...
    IterateOverMap(map, [](auto& map, int r, int c) {
    ...
    IterateOverMap(map, [&riskLevelSum, &basinId](auto& map, int r, int c){
    ...
    do {
        found = false;
        IterateOverMap(map, [&basinIdSizes, &found](auto& map, int r, int c){
            if (map[r][c].basinId < 0 && map[r][c].value < 9) {
                auto adj = GetAdjacents(map, r, c);
                for (auto &&a : adj) {
                    if (a->basinId >= 0) {
